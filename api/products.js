...
async function readCatalog() {
  ...
  try {
    const result = await get(CATALOG_PATH, { access: 'private', token: BLOB_TOKEN, useCache: false });
    const data = await new Response(result.stream).json();
    return Array.isArray(data) ? data : [];
  } catch {
    return readBaseCatalog();
  }
}
...
async function writeCatalog(products) {
  ...
  await put(CATALOG_PATH, JSON.stringify(products, null, 2), {
    access: 'private', token: BLOB_TOKEN, addRandomSuffix: false, allowOverwrite: true,
...
export default async function handler(req, res) {
  try {
    if (req.method === 'GET') return sendJson(res, { ok: true, products: await readCatalog() });
    ...
    if (body.action === 'reset') {
      const products = await readBaseCatalog();
      await writeCatalog(products);
