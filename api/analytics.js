...
const ANALYTICS_SECRET = process.env.COCOON_ANALYTICS_SECRET || process.env.COCOON_ADMIN_PASSWORD || '';
...
function key() {
  return createHash('sha256').update(ANALYTICS_SECRET).digest();
}

function encryptEvent(event) {
  if (!ANALYTICS_SECRET) throw new Error('Missing COCOON_ANALYTICS_SECRET.');
...
function decryptEvent(payload) {
  try {
    const obj = JSON.parse(payload);
    if (obj.v !== 1 || !obj.iv || !obj.tag || !obj.data || !ANALYTICS_SECRET) return null;
    const decipher = createDecipheriv('aes-256-gcm', key(), Buffer.from(obj.iv, 'base64'));
    decipher.setAuthTag(Buffer.from(obj.tag, 'base64'));
    const plain = Buffer.concat([decipher.update(Buffer.from(obj.data, 'base64')), decipher.final()]).toString('utf8');
...
export default async function handler(req, res) {
  try {
    if (!BLOB_TOKEN) return sendJson(res, { ok: false, error: 'Missing BLOB_READ_WRITE_TOKEN.' }, 500);
    if (!ANALYTICS_SECRET) return sendJson(res, { ok: false, error: 'Missing COCOON_ANALYTICS_SECRET.' }, 500);
    ...
    if (req.method === 'POST') {
      ...
      await put(`cocoon/analytics/${stamp}-${Math.random().toString(36).slice(2)}.json`, encryptEvent(event), {
    ...
    if (req.method === 'GET') {
      ...
      do {
        ...
        for (const b of page.blobs) {
          ...
          try {
            const result = await get(b.pathname, { access: 'public', token: BLOB_TOKEN, useCache: false });
            const text = await new Response(result.stream).text();
            const event = decryptEvent(text);
