...
export default async function handler(req) {
  try {
    if (req.method === 'POST') {
      ...
      await put(`cocoon/analytics/${stamp}-${Math.random().toString(36).slice(2)}.json`, JSON.stringify(event), { access:'private', token:BLOB_TOKEN, addRandomSuffix:false, contentType:'application/json' });
      return json({ ok:true });
    }
    if (req.method === 'GET') {
      ...
      do {
        ...
        for (const b of page.blobs) {
          ...
          try { const result = await get(b.pathname, { access:'private', token:BLOB_TOKEN, useCache:false }); events.push(await new Response(result.stream).json()); } catch {}
        }
        cursor = page.hasMore ? page.cursor : undefined;
      } while (cursor);
      events.sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp));
      return json({ok:true,events,days});
    }
    return json({ok:false,error:'Method not allowed'},405);
  } catch (error) { console.error(error); return json({ok:false,error:error?.message || 'Server error'},500); }
}
